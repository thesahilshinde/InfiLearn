...
if(isset($_COOKIE['email']) && $_COOKIE['key']){
    ...
    $decrypted_CookieEmail=openssl_decrypt($_COOKIE['email'],"AES-128-ECB",$EncryptionKey);
    $decrypted_CookiePass=openssl_decrypt($_COOKIE['key'],"AES-128-ECB",$EncryptionKey);

    $time = $currentTime;
    $userEmail = $decrypted_CookieEmail;
    $password = $decrypted_CookiePass;
    ...
    $result = $conn->query($sql);
        if($result->num_rows > 0) {
            ...
            if($isLoggedIn == true){
                $Login_logs_sql =  "INSERT INTO login_logs(userid,role,time,device,ip_address,device_details,browser,device_name,email)
                                    VALUES('$userId','$role','$time','$UserDevice','$ipAddress','$userSystemInfo','$userBrowser','$Device_name','$db_Email')";
